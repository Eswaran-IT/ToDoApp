    ...
    <div className="h-screen bg-purple-900 px-4 py-8 overflow-y-auto">
      ...
      <div className="flex overflow-x-scroll snap-x">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            animate={{ opacity: [0, 1], x: [10, 0] }} // Fade-in and slide-left animation
            transition={{
              duration: 0.8,
              ease: "easeInOut",
            }}
            className="w-full md:w-1/2 xl:w-1/3 2xl:w-1/4 snap-center bg-white rounded-lg shadow-xl overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:bg-gray-50 mx-4"
          >
