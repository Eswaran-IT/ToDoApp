...
function Projects() {
  return (
    <div className="h-screen bg-purple-900 px-4 py-8 overflow-y-scroll">
      <h1 className="text-5xl text-center font-bold mb-12 text-white">My Projects</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
        {projects.map((project, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:bg-purple-100">
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="relative flex flex-col h-full group">
              ...
              <div className="p-6 flex-grow">
                <h2 className="text-2xl font-semibold text-center mb-4 text-purple-600 group-hover:text-purple-800 transition duration-300">{project.title}</h2>
                <p className="text-gray-600 text-center">{project.description}</p>
              </div>
              <div className="mb-6 text-center mt-6">
                <a href={project.link} target="_blank" rel="noopener noreferrer">
                  <FaGithub className="w-8 h-8 mx-auto text-gray-700 hover:text-white transition duration-300 group-hover:text-purple-600 cursor-pointer" />
                </a>
              </div>
            </a>
          </div>
