...
function Projects() {
  return (
    <div className="h-screen bg-white px-4 py-8 overflow-y-auto">
      <h1 className="text-5xl text-center font-bold mb-12 text-purple-700">My Projects</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            animate={{ y: [0, 20, 0] }} // Animating the card's vertical position
            transition={{
              duration: 2, // Duration for the entire cycle (up and down)
              ease: "easeInOut", // Smooth transition
              repeat: Infinity, // Repeat indefinitely
              repeatDelay: 1, // Delay before repeating the animation
            }}
            className="bg-white text-purple-700 rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-xl hover:bg-gray-50"
          >
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="relative flex flex-col h-full group">
              <img
                src={project.icon}
                alt={project.title}
                className="w-24 h-24 mx-auto mt-6 group-hover:opacity-80 transition-opacity duration-300"
              />
              <div className="p-6 flex-grow">
                <h2 className="text-2xl font-semibold text-center mb-4 text-purple-700 group-hover:text-purple-800 transition duration-300">{project.title}</h2>
