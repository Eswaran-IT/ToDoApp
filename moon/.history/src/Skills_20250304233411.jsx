...
const Skills = () => (
  <div className="w-full min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex flex-col items-center justify-center px-6">
      ...
      {skills.map((skill) => {
        ...
        return (
          <motion.div
            ...
            className="flex justify-center items-center transform hover:scale-105 hover:shadow-lg hover:rotate-3 transition-all duration-300"
          >
            <a
              href={skill.link}
              target="_blank"
            ...
            >
              <img
                src={skill.imgSrc}
                alt={`${skill.name} Logo`}
                className="h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 lg:h-28 lg:w-28 object-contain transition-all duration-300"
                onError={(e) => e.target.src = "/fallback-image.svg"} // Fallback in case of image error
              />
            </a>
          </motion.div>
        );
