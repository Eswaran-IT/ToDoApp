      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {projects.map((project, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden transition transform hover:scale-105 hover:shadow-xl">
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="relative flex flex-col h-full">
              <img
                src={project.icon}
                alt={project.title}
              ...
              <div className="p-6 flex-grow">
                <h2 className="text-xl font-semibold text-center mb-4 text-purple-600">{project.title}</h2>
                <p className="text-gray-600 text-center">{project.description}</p>
              </div>
              {/* Small Gap Below Text before GitHub Icon */}
              <div className="mb-text-center">
                <a href={project.link} target="_blank" rel="noopener noreferrer">
                  <FaGithub className="w-8 h-8 mx-auto text-black hover:text-white transition duration-300" />
                </a>
              </div>
            </a>
          </div>
