...
const Certify = () => {
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 flex flex-col items-center justify-center p-0">
      ...
      <div className="w-full max-w-md p-6 mb-6 bg-white bg-opacity-80 text-gray-900 rounded-lg shadow-xl text-center">
      ...
      <div className="w-full flex flex-wrap justify-center gap-3 mb-6">
        {Object.keys(certificateMap).map((category, index) => (
          <button
            key={index}
            className={`px-3 py-2 text-sm text-white font-semibold rounded-md transition duration-200 ease-in-out ${activeCategory === category ? 'bg-purple-600' : 'bg-gray-700 hover:bg-purple-500'}`}
            ...
            {certificates.length > 1 && (
              <>
                <button
                  ...
                  className="absolute top-1/2 left-4 transform -translate-y-1/2 text-white bg-purple-600 p-3 rounded-full opacity-70 hover:opacity-100 transition duration-200"
                >
                  <FaArrowLeft />
                </button>
                <button
                  onClick={() => setCurrentIndex((prevIndex) => (prevIndex + 1) % certificates.length)}
                  className="absolute top-1/2 right-4 transform -translate-y-1/2 text-white bg-purple-600 p-3 rounded-full opacity-70 hover:opacity-100 transition duration-200"
                >
                  <FaArrowRight />
                </button>
              </>
