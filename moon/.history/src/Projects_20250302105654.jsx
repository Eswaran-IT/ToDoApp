    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {projects.map((project, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden transform transition duration-500 hover:scale-105 hover:shadow-2xl hover:bg-purple-50">
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="relative flex flex-col h-full group">
              <img
                src={project.icon}
                alt={project.title}
                className="w-20 h-20 mx-auto mt-4 group-hover:opacity-80 transition-opacity duration-300"
              />
              <div className="p-6 flex-grow">
                <h2 className="text-xl font-semibold text-center mb-4 text-purple-600 group-hover:text-purple-800 transition duration-300">{project.title}</h2>
                <p className="text-gray-600 text-center">{project.description}</p>
              </div>
              <div className="mb-4 text-center mt-4">
                <a href={project.link} target="_blank" rel="noopener noreferrer">
                  <FaGithub className="w-8 h-8 mx-auto text-black hover:text-white transition duration-300 group-hover:text-purple-600 cursor-pointer" />
                </a>
              </div>
            </a>
          </div>
